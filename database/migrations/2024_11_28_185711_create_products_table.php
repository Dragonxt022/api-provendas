...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('products', function (Blueprint $table) {
            ...
            $table->string('sku', 50)->unique(); // SKU (código único do produto)
            $table->string('barcode', 50)->nullable(); // Código de barras (opcional)
            $table->text('description')->nullable(); // Descrição detalhada (opcional)
            $table->foreignId('category_id')->constrained()->onDelete('cascade'); // Relacionamento com a categoria
            ...
            $table->foreignId('supplier_id')->nullable()->constrained()->onDelete('set null'); // Relacionamento com fornecedor (opcional)
